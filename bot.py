...
def get_types(subject):
    ...
    return folders  # لو مفيش أنواع، هيرجع []
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔙 رجوع للقائمة السابقة":
        if "lecture" in state:
            ...
            keyboard = [[l] for l in lectures] + [["🔙 رجوع للقائمة السابقة"], ["🔁 ابدأ من جديد"]]
        ...
        elif "type" in state and state["type"]:
            types = get_types(state["subject"])
            keyboard = [[t] for t in types] + [["🔙 رجوع للقائمة السابقة"], ["🔁 ابدأ من جديد"]]
            ...
            keyboard = [[s] for s in get_subjects()] + [["🔁 ابدأ من جديد"]]
            del user_state[uid]["subject"]
            await update.message.reply_text("📚 اختر المادة:", reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True))
        else:
            await update.message.reply_text("❗ لا يوجد خطوة سابقة.")
        return
    ...
    if text in get_subjects():
        ...
        lecture_key = text.replace(".pdf", "").strip()
        user_state[uid]["lecture"] = lecture_key
        keyboard = [["📄 View Lecture File", "📝 Take Quiz"], ["🔙 رجوع للقائمة السابقة"], ["🔁 ابدأ من جديد"]]
